...
public class AsaDefaultManagementMetadataProvider implements ManagementMetadataProvider {
	...
	public ManagementMetadata get(EurekaInstanceConfigBean instance, int serverPort, String serverContextPath,
        ...
        // Use secure port as management port when secure port is enabled.
        // The management port will affect the 'managementUrl' showed on instance detail page.
        if (managementPort == null && instance.getSecurePortEnabled()) {
            managementPort = instance.getSecurePort();
        }
		...
		if (instance.isSecurePortEnabled()) {
            // Use secure port instead of server port.
			metadata.setSecureHealthCheckUrl(getHealthCheckUrl(instance, instance.getSecurePort(), serverContextPath,
